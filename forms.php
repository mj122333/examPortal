...
if (isset($_GET['exam_id']) && $exam_id > 0) {
    ...
    try {
        ...
        if (!$allAnswers || count($allAnswers) == 0) {
            die("Nisu pronađeni odgovori za ovaj ispit. ID ispita: " . $exam_id);
        }

        // Ispiši podatke za debugging (možete ukloniti kasnije)
        // echo "<pre>Exam ID: " . $exam_id . "\nNađeno odgovora: " . count($allAnswers) . "\nPrvi odgovor: ";
        // print_r($allAnswers[0]);
        // echo "</pre>";
        ...
        foreach ($allAnswers as $row) {
            // Pokušaj dohvatiti dodatne podatke iz ep_pitanje na temelju teksta pitanja
            // (ovo je rezervni pristup, u slučaju da ne postoji direktna veza)
            $stmtPitanje = $conn->prepare("
                SELECT * FROM ep_pitanje 
                WHERE tekst_pitanja LIKE :question_text
                LIMIT 1
            ");
            $stmtPitanje->execute([':question_text' => "%" . trim($row["question_text"]) . "%"]);
            $pitanjeData = $stmtPitanje->fetch(PDO::FETCH_ASSOC);
            
            // Pripremi podatke za prikaz
            $explanationText = !empty($row["explanation"]) ? $row["explanation"] : "Nema dodatnog objašnjenja.";
            
            // Ako postoje podaci iz tablice pitanja, koristi ih za dodatne informacije
            if ($pitanjeData) {
                $explanationText = !empty($pitanjeData["hint"]) ? $pitanjeData["hint"] : 
                                (!empty($row["explanation"]) ? $row["explanation"] : 
                                (!empty($pitanjeData["obrazlozenje"]) ? $pitanjeData["obrazlozenje"] : "Nema dodatnog objašnjenja."));
                ...
                $entry = [
                    ...
                    "options"        => [
                        $pitanjeData["odgovor1"] ?? "",
                        $pitanjeData["odgovor2"] ?? "",
                        $pitanjeData["odgovor3"] ?? "",
                        $pitanjeData["odgovor4"] ?? ""
                    ],
                    "correct_index"  => ($pitanjeData["tocan_odgovor"] ?? 1) - 1,
                    "explanation"    => $explanationText
                ];
            } else {
                // Ako nema podataka iz tablice pitanja, koristi samo podatke iz odgovora
                $entry = [
                    "question"       => $row["question_text"],
                    "your_answer"    => $row["user_answer_text"],
                    "correct_answer" => $row["correct_answer_text"],
            ...
            if (isset($row["is_correct"]) && $row["is_correct"] == 1) {
