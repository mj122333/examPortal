...
if (isset($_GET['getQuestions']) && $_GET['getQuestions'] == 1) {
    ...
    $_SESSION['quiz_answers'] = []; // Ovdje ćemo pohraniti točne odgovore za serversku validaciju
    $questions = [];
    foreach ($questionsData as $i => $q) {
        ...
        if (count($answersRows) == 0) {
            continue; // Preskočimo pitanja bez odgovora
        }

        // Randomizacija odgovora
        shuffle($answersRows);
        ...
        // Slažemo odgovore nakon shufflea
        foreach ($answersRows as $row) {
            $answers[] = $row['tekst'];
            $answerIds[] = $row['ID'];
            if ($row['tocno'] == 1) {
                $correctAnswerId = $row['ID'];
            }
        }

        // Pohrani informacije o pitanju i odgovorima u sesiju
        $_SESSION['quiz_answers'][$questionId] = [
            'answerIds' => $answerIds,
            'correctAnswerId' => $correctAnswerId
        ];

        // Povezivanje odgovora u pipe-odvojeni string
        $answersPipe = implode('|', $answers);
        $answerIdsPipe = implode('|', $answerIds);

        // Stvaranje objekte pitanja (bez informacije koji je odgovor točan)
        $questions[] = [
            'questionId'    => $questionId,
            'question'      => $q['tekst_pitanja'],
            'answers'       => $answersPipe,
            'answerIds'     => $answerIdsPipe,
            'image'         => $q['slika'] ?? '',
            'hint'          => $q['hint'] ?? ''
        ];
    }
...
<body>
  ...
  <script>
    ...
    function loadQuestion() {
      ...
      const answerIdsArray = currentQuestion.answerIds.split('|');
      ...
      answersArray.forEach((answer, index) => {
        if (answer && answer.trim() !== '') {
          ...
          button.dataset.answerId = answerIdsArray[index];
    ...
    function selectAnswer(e) {
      ...
      allButtons.forEach(button => {
        ...
        button.disabled = true; // Onemogući sve gumbe nakon odabira
      });
      ...
      // Spremi odgovor korisnika (bez informacije je li točan)
